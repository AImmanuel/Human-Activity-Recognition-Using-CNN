   ...
   #11Feb         
    def compute_optical_flow(self, prev_frame, current_frame):
        ...
        contours_1, _ = cv2.findContours(
                    current_frame_masked, 
                    cv2.RETR_EXTERNAL, 
                    cv2.CHAIN_APPROX_SIMPLE
                    )
        # Draw contours on a blank image
        contour_img_c = np.zeros_like(current_frame_masked)
        cv2.drawContours(contour_img_c, contours_1, -1, (255, 255, 255), thickness=1)
        ...
        # Draw contours on a blank image
        contour_img_p = np.zeros_like(prev_frame_masked)

        cv2.drawContours(contour_img_p, contours_2, -1, (255, 255, 255), thickness=1)
        #binary_edge_image = cv2.threshold(contour_img, 1, 255, cv2.THRESH_BINARY)[1]

        #original
        #concatenated_frames = cv2.hconcat([current_frame, edge_frame_curr_v2])

        concatenated_frames = cv2.hconcat([current_frame, contour_img_c])
        cv2.imshow('Original vs Preprocessed Frames', concatenated_frames)
        cv2.waitKey(1)
        
        prev_frame = cv2.medianBlur(contour_img_p, 5)
        current_frame = cv2.medianBlur(contour_img_c, 5)
        #====================================================================
...
if __name__ ==  "__main__":
    dataset_folder = "D:/MscProject/UP-Fall"
