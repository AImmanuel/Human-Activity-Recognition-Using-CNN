...
class FallDetectionCNN(nn.Module):
    ...
    def forward(self, x):
        x = F.gelu(self.atn1(self.bn1(self.conv1(x))))
        x = F.max_pool3d(x, 2)

        x = F.gelu(self.atn2(self.bn2(self.conv2(x))))
        x = F.max_pool3d(x, 2)

        x = F.gelu(self.atn3(self.bn3(self.conv3(x))))
        x = F.max_pool3d(x, 2)
...
if __name__ == "__main__": 
    ...
    saved_model_path = 'CBAM_model_3d.pth'
    ...
    torch.save(model.state_dict(), 'CBAM_model_3d_1to1.pth')
